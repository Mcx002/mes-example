<template>
    <div class="card-body" style="overflow-x:scroll">
                        ...
                        <form class="form-material" @submit.prevent="actDataChild">
                            <div class="modal-body">
                                ...
                                <div class="row">
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <input class="form-control" type="date" v-model="dateinputdefect" required />
                                        </div>
                                    </div>
                                </div>
                        ...
                        <form class="form-material" @submit.prevent="actDataChild">
                            <div class="modal-body">
                                ...
                                <div class="row">
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label for="defectinputdate" class="control-label">Defect date:</label>
                                            <input required class="form-control" type="date" id="defectinputdate" v-model="dateinputdefect" />
                                        </div>
                                    </div>
                                </div>
                                <div class="row" v-for="(form,f) in childFormData" v-show="!fistloadchildform">
        ...
        <div v-else class="card text-muted text-center mt-5 mb-3">
...
export default{
    ...
    methods:{
        ...
        getDataListDefectInfo(){
            axios.get('/defectinfo')
                .then(response=>{
                    this.listdefectinfo = response.data
                })
        },
        getDataLine(){
            axios.get('/lines')
        ...
        childForm(wo,qty,item){
            ...
            item.forEach((val,i)=>{
                ...
                this.prodlineselection.forEach((pls,a)=>{
                    if(val.c_op==pls.id){
                        pls.selection.forEach((plsselect=>{
                            selection.push({'id':plsselect,'title':plsselect})
                        }))
                    }
                })
                var selectdefectinfo = []
                if(this.listdefectinfo.length>0){
                    this.listdefectinfo.forEach((ldv,ldi)=>{
                        if(ldv.part_no==wo.part_no&&ldv.c_op==val.c_op){
                            selectdefectinfo.push({'id':ldv.defect_id,'title':ldv.defect_desc})
                        }
                    })
                    // console.log(selectdefectinfo)
                ...
                axios.post('/countWip',{
                    ...
                    if (!res[value.c_op]) {
                        res[value.c_op] = {c_op: value.c_op, operation_id: value.operation_id, prod_line: value.prod_line, qty: 0};
                    ...
                    if(qty.type=='parent'){
                        if(data.length>0){
                            this.childFormDataForSend.forEach((cfv,cfi)=>{
                                ...
                                //     console.log(cfv.location+" "+dtv.c_op)
                                    ...
                                    //     if(sumqtygroupby.hasOwnProperty([1])){
                                    //         this.childFormDataForSend[0].o_qty = data[0].qty-sumqtygroupby[1].qty
                                    //         this.childFormData[0][3].max = data[0].qty-sumqtygroupby[1].qty
                                    //     }else{
                                    ...
                                    //     }
                                    // }
                                //     if(sumqtygroupby.hasOwnProperty([dti+1])){
                                //         this.childFormDataForSend[cfi].o_qty = sumqtygroupby[dti+1].qty
                                //         this.childFormData[cfi][3].max = sumqtygroupby[dti+1].qty
                                //     }else{
                                //         this.childFormDataForSend[cfi].o_qty = 0
                                //         this.childFormData[cfi][3].max = 0
                                //     }
                                // })
                                
                                if(sumqtygroupby.hasOwnProperty([cfi+1])){
                                    this.childFormDataForSend[cfi].o_qty = sumqtygroupby[cfi+1].qty
                                    this.childFormData[cfi][3].max = sumqtygroupby[cfi+1].qty
                                }else{
                                    this.childFormDataForSend[cfi].o_qty = 0
                                    this.childFormData[cfi][3].max = 0
                                }
                        ...
                        if(data.length>0){
                            this.childFormDataForSend.forEach((cfv,cfi)=>{
                                // sumqtygroupby.forEach((dtv,dti)=>{
                                //     // console.log(sumqtygroupby.hasOwnProperty([dti+1]))
                                //     // if(cfi==0){
                                //     //     this.childFormDataForSend[0].o_qty = data[0].qty
                                //     //     this.childFormData[0][3].max = data[0].qty
                                //     // }else{
                                ...
                                //     if(sumqtygroupby.hasOwnProperty([dti+1])){
                                //         this.childFormDataForSend[dti].o_qty = sumqtygroupby[dti+1].qty
                                //         this.childFormData[dti][3].max = sumqtygroupby[dti+1].qty
                                //     }else{
                                //         this.childFormDataForSend[cfi].o_qty = 0
                                //         this.childFormData[cfi][3].max = 0
                                //     }
            ...
            this.childFormData.forEach((item,i)=>{
                ...
                // this.childFormData[i][4].selection = this.childFormData[i][2].selection
            })
        },
        ...
        isProdLineRequired(item,form,f,i){
            return (item.id=='prod_line'&&!(item.type=='select'&&item.selection.length==0)&&this.childFormDataForSend[f][form[i+1].id]!='')
        },
        isGetProcessRequired(item,form,f,i){
            return ((item.id=='get_process'&&item.selection.length!=0)&&this.childFormDataForSend[f][form[i+2].id]!='')
        },
        isDefectInfoRequired(item,form,f,i){
            return ((item.id=='defect_info_id'&&item.selection.length!=0)&&this.childFormDataForSend[f][form[i-1].id]!='')
        }

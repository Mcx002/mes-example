<template>
    ...
    <div class="card-body" style="overflow-x:scroll">
        ...
        <div class="row">
            ...
            <div class="col-md-3 form-group">
                <label for="part_no">SET UPPER</label>
                <select class="form-control" id="part_no" v-model="search.part_no">
                    <option v-for="item in uppersets">{{item.part}}</option>
                </select>
            </div>
        </div>
        <div v-if="showdata">
...
export default{
    ...
    methods:{
        ...
        getDataBom(){
            axios.get('/boms')
                .then(({data})=>{
                    var part = data.filter(item=>item.bom_parent!==null).map(item=>{
                        var result = item.bom_parent.split('-')
                        return {
                            bom_parent:result[result.length-1]
                        }
                    })
                    var part_parent = _.chain(part).groupBy('bom_parent').map((item,i)=>{
                        return {
                            part:i,
                        }
                    }).value()
                    this.uppersets = part_parent
